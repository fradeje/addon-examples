...
async function fetchEcbUsdToEur() {
    ...
    // Extract USD per EUR
    // Example: <Cube currency='USD' rate='1.1234'/>
...
module.exports.registerInvoicePdfEndpoint = function registerInvoicePdfEndpoint(app) {
    app.get("/invoice.pdf", async (req, res) => {
        ...
        try {
            ...
            // Base rate in USD (your UI shows "USD X.00 × FX = EUR ...")
            const rateUsd = Number(profile.rate || 0);
            ...
            // Decide invoice currency + rate
            let currency = "USD";
            let rateToUse = rateUsd;
            ...
            if (selectedCurrency === "EUR") {
                ...
                currency = "EUR";
                rateToUse = rateUsd * eurPerUsd;

                // For display/comment: keep 6 decimals for FX to match your UI
                const fxStr = eurPerUsd.toFixed(6);
                const eurRateStr = rateToUse.toFixed(2);
                conversionComment = `USD ${rateUsd.toFixed(2)} × ${fxStr} = EUR ${eurRateStr}${fxDate ? ` (ECB ${fxDate})` : ""}`;
            }

            // Amount computed from rateToUse in invoice currency
            const subtotalCents = moneyCentsFrom(hours, rateToUse);
            ...
            if (vatPercent > 0) {
                doc.text(`IGIC (${vatPercent.toFixed(2)}%)`, totalsX, y, { width: totalsW * 0.5, align: "left" });
            ...
            // Comments (conversion info) — only when EUR
            ...
            // Notes (free text from settings)
            const notes = String(profile.notes || "").trim();
            if (notes) {
                y += 22;
                doc.font("Helvetica-Bold").fontSize(11).fillColor("#111").text("Notes", left, y);
                y += 10;
                ...
                const textHeight = doc.heightOfString(notes, { width: textWidth });
                const boxHeight = Math.max(60, textHeight + boxPadding * 2);
                ...
                doc.text(notes, left + boxPadding, y + boxPadding, { width: textWidth });
                y += boxHeight;
            }
